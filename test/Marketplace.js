...
contract("NFTmarketplace", (accounts) => {
    ...
    describe(`NFTmarketplace contract address`, async () => {
        ...
        it(`Trade`, async () => {
            await ERC721Instance.setApprovalForAll(
                proxyInstance.address,
                true,
                { from: accounts[1] }
            );; // approval to transfer from proxy
            const getfee = await tradeInstance.getFees([accounts[0], accounts[1], tokenInstance.address, ERC721Instance.address, 1, 1, 1, "20000000000000000", 2, 'sample', 1, 10, 1]);
            console.log(getfee);
            ...
            const mint = await ERC721Instance.createToken('sample', "10000000000000000", { from: accounts[1], value: "10000000000000000" });
            const event = mint.receipt.logs[0];
            tokenId = Number(event.args[2]);
            await ERC721Instance.executeSale(tokenId, {
                from: accounts[0],
                value: "10000000000000000",
            });;
            await proxyInstance.changeOperator(tradeInstance.address, {
                from: accounts[0],
            });; // only  once after deployment of contract
            ...
            console.log(owner);
            await ERC721Instance.approve(tradeInstance.address, 1, {
                from: accounts[0],
            });;
            await tradeInstance.executeBid(
                [
                    accounts[0],
                    accounts[1],
