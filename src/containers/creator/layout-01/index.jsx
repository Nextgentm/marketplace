...
const CreatorArea = ({ className, space, id, data }) => {
  ...
  const filterHandler = useCallback(() => {
    ...
    const filterdSellers = allSellers.filter((seller) => slugify(seller.top_since) === slugify(current));
  ...
  return (
    <div className={clsx("rn-creator-title-area", space === 1 && "rn-section-gapTop", className)} id={id}>
      <div className="container">
        <div className="row align-items-center">
          <div className="col-lg-6 col-md-6 col-sm-6 col-12">
            {data?.section_title && <SectionTitle {...data.section_title} className="mb--0" disableAnimation />}
          </div>
        ...
        <div className="row g-5 mt--30 creator-list-wrapper">
          {sellers.slice(0, 8).map((seller) => (
            <div key={seller.id} className="creator-single col-lg-3 col-md-4 col-sm-6">
              <TopSeller
                className="explore"
                name={seller.name}
