...
const CollectionArea = ({ className, space, id, data }) => {
  ...
  useEffect(() => {
    ...
    if (collectionApiData?.collections) {
      console.log("collectionApiData?.collections", collectionApiData?.collections);
      setPagination({
        ...collectionApiData?.collections?.meta?.pagination,
        total: collectionApiData.collections?.data.length,
        pageCount: Math.ceil(collectionApiData.collections?.data.length / 8)
      });
