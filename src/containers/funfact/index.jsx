...
const FunfactArea = ({ space, className, data }) => (
  <div className={clsx("rn-statistick-area", space === 1 && "rn-section-gapTop", className)}>
    <div className="container">
      <div className="row mb--30">
        <div className="col-12 text-center">{data?.section_title && <h3>{data.section_title.title}</h3>}</div>
      </div>
          ...
          <div className="row g-5">
            {data?.funfacts?.map((funfact) => (
              <div key={funfact.id} className="col-md-6">
                <FunFact counter={funfact.counter} title={funfact.title} suffix={funfact.suffix} />
              </div>
            ))}
          </div>
        </div>
