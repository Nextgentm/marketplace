        ...
        <div className="col-lg-12">
          <div className="banner-xii-wrapper">
            <div className="banner-content-wrapper">
              {headings?.[0]?.content && <p className="pre-title">{headings[0].content}</p>}
              {headings?.[1]?.content && <h1 className="title">{headings[1].content}</h1>}
              {texts?.map((text) => (
              ...
              <div className="wallet-image-wrapper">
                {items?.map(({ id, title, path, image: walletImg }) => (
                  <Anchor key={id} path={path} className="avatar" data-tooltip={title}>
                    {walletImg?.src && (
                      <Image
                        src={walletImg.src}
