...
const HeroArea = ({ data: { image, headings, highest_bid, auction_date, categories, properties } }) => (
  <div
      ...
      <div className="row g-5 d-flex align-items-center flex-wrap">
        {image?.src && (
          <div className="col-xxl-6 col-xl-6 col-lg-12 col-md-12">
            <Tilt tiltReverse tiltMaxAngleX={3} tiltMaxAngleY={3} perspective={500} gyroscope>
              <div className="left-thumbnail-image">
                <Image src={image.src} alt={image?.alt || "banner"} width={886} height={886} priority />
              </div>
            </Tilt>
          </div>
        )}
        <div className="col-xxl-5 col-xl-6 col-lg-12 col-md-12">
          {headings?.[0]?.content && <h1 className="title mb--30">{headings[0].content}</h1>}
          <PlaceBet
          ...
          <div className="d-flex">
            <div className="rn-pd-sm-property-wrapper into-banner">
              ...
              <div className="catagory-wrapper">
                {categories?.map((cat) => (
                  <ProductProperty key={cat.id} type={cat.type} value={cat.value} />
                ))}
              </div>
            </div>
            <div className="rn-pd-sm-property-wrapper into-banner ml--30">
              ...
              <div className="property-wrapper">
                {properties?.map((prop) => (
                  <ProductProperty key={prop.id} type={prop.type} value={prop.value} />
                ))}
              </div>
            </div>
          </div>
