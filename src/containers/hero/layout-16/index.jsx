...
import { HeadingType, TextType, ButtonType, ItemType, ImageType } from "@utils/types";

const HeroArea = ({ data: { headings, texts, buttons, image, items, clients } }) => (
          ...
          <div className="left-banner-16-wrapepr mt_md--100 mt_sm--100">
            ...
            <div className="button-group d-flex flex-wrap">
              {buttons?.map(({ content, id, ...btn }, i) => (
                <Button {...btn} key={id} className={i !== buttons.length - 1 ? "mr--30" : ""}>
                  {content}
                </Button>
              ))}
            </div>
              ...
              <div className="joined-people-wrapper">
                <div className="product-share-wrapper">
                  <div className="profile-share">
                    {clients?.map((client) => (
                      <Anchor path={client.path} className="avatar" data-tooltip={client.name} key={client.id}>
                        {client?.image?.src && (
                          <Image
                            className="large"
                    ...
                    <Anchor className="more-author-text" path="/author" tabIndex="0">
                      More Then 25K
                    </Anchor>
                  </div>
                </div>
