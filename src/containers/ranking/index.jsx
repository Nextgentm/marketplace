...
const RankingArea = ({ className, space, data }) => {
  ...
  return (
    <div className={clsx("rn-upcoming-area", space === 1 && "rn-section-gapTop", className)}>
              ...
              <table className="table upcoming-projects">
                ...
                <tbody className="ranking">
                  {ranking?.map((item, index) => (
                    <tr key={item.id} className={index % 2 === 0 ? "color-light" : ""}>
                      <td>
                        <span>{index + 1}.</span>
                      </td>
                      <td>
                        <div className="product-wrapper d-flex align-items-center">
                          {item?.product?.image?.src && (
                            <Anchor path={item.product.slug} className="thumbnail">
                              <Image src={item.product.image.src} alt="Nft_Profile" width={56} height={56} />
                            </Anchor>
                          )}
                      ...
                      <td>
                        <span className={item["24h%"].status === "-" ? "color-danger" : "color-green"}>
                          {item["24h%"].status}
                          {item["24h%"].charge}
                        </span>
                      </td>
                      <td>
                        <span className={item["7d%"].status === "-" ? "color-danger" : "color-green"}>
                          {item["7d%"].status}
                          {item["7d%"].charge}
                        </span>
                      </td>
