    ...
    <TabContainer defaultActiveKey="nav-0">
      <div className="pd-tab-inner">
        <Nav className="rn-pd-nav rn-pd-rt-content nav-pills">
          {images.data && (
            <Nav.Link
              key={images.data.url || "/images/portfolio/lg/portfolio-01.jpg"}
              as="button"
              eventKey={`nav-${images.data.id}`}
            >
              <span className="rn-pd-sm-thumbnail">
                <Image
                  src={images.data.url || "/images/portfolio/lg/portfolio-01.jpg"}
                  alt={images.data.alternativeText || "Product"}
                  width={167}
                  height={167}
                />
              </span>
        ...
        <TabContent className="rn-pd-content">
          {images.data && (
            <TabPane key={images.data.url} eventKey={`nav-${images.data.id}`} active="true">
